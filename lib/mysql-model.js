...
//{ userId:18, userInfos: { userId: '=' }, comments: { userId: '=' , likes: { commentId: '=' } } }
...
MySQLModel.prototype.parseTable = function (mysqlTableToSearch, parentObject) {
    ...
    var childResultsLen = 0;
    _model.map(parentObject).then(function (results) {
        childResultsLen++;
        
        /*if (results.length === 1) {
            results = results[0];
            
            if (tableProperty.slice(-1) === 's') {
                delete parentObject[tableProperty];
                tableProperty = tableProperty.slice(0, -1);
        ...
        //results.length === undefined gia na dw an einai ena mono object, ara kateu9eian resolve
        //     if (results.length === undefined || results.length ===1 || childResultsLen === results.length) {
        
        //console.log('finish table ' + mysqlTableToSearch);
        newdef.resolve();
       // }
...
//test gia to find einai auto, den doulevei tpt kai trelelee...
MySQLModel.prototype.map = function (parentObj) {
    var def = Promise.defer();
    var self = this;
    if (!parentObj) {
    ...
    //self.jsObject = {userId:18, comments {userId :'='}
